...
static struct string_state		share_string_state;
static struct unstring_state	share_unstring_state;
static struct inspect_state	 	*pshare_inspect_state  = &share_inspect_state;
...
COB_INLINE COB_A_INLINE void
cob_inspect_init_common_mt (struct inspect_state **pst, cob_field *var)
{
	if (*pst == NULL) {
		*pst = malloc (sizeof(struct inspect_state));
		(*pst)->mark = NULL;
		(*pst)->repdata = NULL;
	}
	struct inspect_state *st = *pst;
...
void
cob_inspect_init_mt (struct inspect_state **pst, cob_field *var, const cob_u32_t replacing)
{
...
void
cob_inspect_init_converting_mt (struct inspect_state **pst, cob_field *var)
{
	cob_inspect_init_common_mt (pst, var);
	struct inspect_state *st = *pst;
...
{
	cob_inspect_init_converting_mt (&pshare_inspect_state, var);
}

void
cob_inspect_start_mt (struct inspect_state *st)
{
	st->start = st->data;
...
{
	cob_inspect_start_mt (&share_inspect_state);	
}
...
{
	cob_inspect_before_mt (&share_inspect_state, str);
}
...
{
	cob_inspect_after_mt (&share_inspect_state, str);
}

void
cob_inspect_characters_mt (struct inspect_state *st, cob_field *f1)
{
	const size_t	pos = st->start - st->data;
	const size_t	len = st->end - st->start;
...
{
	cob_inspect_characters_mt(&share_inspect_state, f1);
}
...
{
	cob_inspect_converting_mt (&share_inspect_state, f1, f2);
}

/* note: currently not used by cobc (disabled unfinished prototype) */
void
cob_inspect_translating_mt (struct inspect_state *st, const unsigned char *conv_table)
{
	const size_t	len = st->end - st->start;
...
{
	cob_inspect_translating_mt (&share_inspect_state, conv_table);
}

void
cob_inspect_finish_mt (struct inspect_state *st)
{
...
{
	cob_inspect_finish_mt (&share_inspect_state); 
}
...
void
cob_string_init_mt (struct string_state **pst, cob_field *dst, cob_field *ptr)
{
	if (*pst == NULL)
		*pst = malloc (sizeof(struct string_state));
	struct string_state *st = *pst;
...
{
	cob_string_init_mt (&pshare_string_state, dst, ptr);
}

void
cob_string_delimited_mt (struct string_state *st, cob_field *dlm)
{
	if (dlm) {
		st->dlm_copy = *dlm;
		st->dlm = &st->dlm_copy;
...
{
	cob_string_delimited_mt (&share_string_state, dlm);
}

void
cob_string_append_mt (struct string_state *st, cob_field *src)
{
	size_t	src_size;
	int	i;
	int	size;
...
{
	cob_string_append_mt (&share_string_state, src);
}

void
cob_string_finish_mt (struct string_state *st)
{
	if (st->ptr) {
...
{
	cob_string_finish_mt (&share_string_state);
}
...
void
cob_unstring_init_mt (
	struct unstring_state **pst,
	cob_field *src,
	cob_field *ptr,
...
{
	if (*pst == NULL) {
		*pst = malloc (sizeof(struct unstring_state));
		(*pst)->dlm_list = NULL;
	}
	struct unstring_state *st = *pst;
...
void
cob_unstring_delimited_mt (struct unstring_state *st, cob_field *dlm, const cob_u32_t all)
{
...
{
	cob_unstring_delimited_mt (&share_unstring_state, dlm, all);
}

void
cob_unstring_into_mt (
	struct unstring_state *st,
	cob_field *dst,
	cob_field *dlm,
	cob_field *cnt
)
...
{
	cob_unstring_into_mt (&share_unstring_state, dst, dlm, cnt);
}

void
cob_unstring_tallying_mt (struct unstring_state *st, cob_field *f)
{
...
{
	cob_unstring_tallying_mt (&share_unstring_state, f);
}

void
cob_unstring_finish_mt (struct unstring_state *st)
{
	if (st->offset < (int)st->src->size) {
...
{
	cob_unstring_finish_mt (&share_unstring_state);
}

/* Initialization/Termination */

void
cob_exit_strings_mt (struct inspect_state *sti, struct unstring_state *stu)
{
	if (sti->mark) {
		cob_free (sti->mark);
		sti->mark = NULL;
