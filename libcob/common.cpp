...
cob_exit_common(void)
{
	...
	if(cobsetptr) {
		if(cobsetptr->cob_config_file) {
			for(unsigned int i = 0; i < cobsetptr->cob_config_num; i++) {
				if(cobsetptr->cob_config_file[i]) {
					delete [] cobsetptr->cob_config_file[i];
				}
			}
		...
		for(int i = 0; i < NUM_CONFIG; i++) {
			if((gc_conf[i].data_type & ENV_STR)
				...
				char ** data = (char **)((char *)cobsetptr + gc_conf[i].data_loc);
				if(*data != NULL) {
					delete [] *data;
					*data = NULL;
				}
...
{
	char * msg = cob_cache_malloc((size_t)COB_ERRBUF_SIZE);
#ifdef HAVE_STRERROR
...
extern "C" DECLNORET void COB_A_NORETURN
...
{
	const char * save_source_file = cob_source_file;
	...
	for(int i = 0; i < NUM_CONFIG; i++) {
		char * env;
		if(gc_conf[i].env_name && (env = getenv(gc_conf[i].env_name)) != NULL) {
			int old_type = gc_conf[i].data_type;
			...
			if(*env != '\0' && set_config_val(env, i)) {
				...
				for(int j = 0; j < NUM_CONFIG; j++) {
					if(j != i && gc_conf[i].data_loc == gc_conf[j].data_loc) {
...
void
cob_check_odo2(const int i, const int min,
...
{
	cob_check_odo2(i, min, max, "variable", name);
}

void
cob_check_subscript2(const int i, const int max,
...
cob_check_subscript(const int i, const int min,
					const int max, const char * name)
{
	cob_check_subscript2(i, max, name, 0);
}
...
{
	HMODULE kernel32_handle = GetModuleHandle(TEXT("kernel32.dll"));
...
cob_sys_hosted(void * p, const void * var)
{
	...
	if(COB_MODULE_PTR->cob_procedure_params[1]) {
		int i = (int) COB_MODULE_PTR->cob_procedure_params[1]->size;
...
cob_sys_calledby(void * data)
{
	...
	if(!COB_MODULE_PTR->cob_procedure_params[0]) {
		/* TODO: check what ACU ccbl/runcbl returns,
...
cob_sys_getopt_long_long(void * so, void * lo, void * idx, const int long_only, void * return_char, void * opt_val)
{
	...
	*/
	...
	if(COB_MODULE_PTR->cob_procedure_params[1]) {
		l = (longoption_def *)(COB_MODULE_PTR->cob_procedure_params[1]->data);
	}
...
{
	if(!strarray || size <= 0 || !separator) {
		return NULL;
	}

	char * result = cob_strdup(strarray[0]);
...
cob_save_env_value(char * env_var, char * env_val)
{
	...
	env_var = new char[strlen(env_val) + 1];
		...
		if(strval[k] == '$' && strval[k + 1] == '{') {
			...
			if(penv != NULL) {
				if((j + strlen(penv)) > (unsigned int)(envlen - 128)) {
					char * s = cob_malloc(strlen(penv) + 256);
					memcpy(s, env, envlen);
					delete [] env;
					env = s;
...
cb_config_entry(char * buf, int line)
{
	...
	while(buf[i] != 0 && (isspace((unsigned char)buf[i]) || buf[i] == ':' || buf[i] == '=')) {
		i++;
	}
	if(buf[i] == '"'
			||  buf[i] == '\'') {
	...
	if(strcasecmp(keyword, "setenv") == 0) {
		...
		k = 0;
		while(value[k] != '=' && value[k] != ':' && value[k] != '"' && value[k] != '\'' && value[k] != 0) {
			k++;
		}
		if(value[k] == '=' || value[k] == ':') {
		...
		while(isspace((unsigned char)buf[i]) || buf[i] == ':' || buf[i] == '=') {
			i++;
		}
		if(buf[i] == '"'
				|| buf[i] == '\'') {
...
cob_load_config_file(const char * config_file, int isoptional)
{
	...
	if(conf_fd != NULL) {
		if(cobsetptr->cob_config_file == NULL) {
			cobsetptr->cob_config_file = new char * [1];
		} else {
			char ** p = new char * [cobsetptr->cob_config_num + 1];
			memcpy(p, cobsetptr->cob_config_file, sizeof(char *) * (cobsetptr->cob_config_num));
			delete [] cobsetptr->cob_config_file;
...
cob_init(const int argc, char ** argv)
{
	...
	cobsetptr = new cob_settings;
	memset(cobsetptr, 0, sizeof(cob_settings));
