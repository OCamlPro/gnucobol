...
dupargv (char * const *argv)
{
  ...
  copy = (char **) cob_malloc ((argc + 1) * sizeof (char *));

  /* the strings */
  for (argc = 0; argv[argc] != NULL; argc++)
    copy[argc] = cob_strdup (argv[argc]);
  copy[argc] = NULL;
  return copy;
}
...
static char **buildargv (const char *input)
{
  ...
  if (input != NULL)
    {
      copybuf = (char *) cob_malloc (strlen (input) + 1);
      ...
      do
	{
	  ...
	  if ((maxargc == 0) || (argc >= (maxargc - 1)))
	      ...
	      if (argv == NULL)
		{
		  ...
		  nargv = (char **) cob_malloc (maxargc * sizeof (char *));
		}
	      else
		{
		  maxargc *= 2;
...
cob_expandargv (int *argcp, char ***argvp)
{
  ...
  while (++i < *argcp)
	...
	goto error;
      buffer = (char *) cob_malloc (pos * sizeof (char) + 1);
...
expandargstr (const char *progname, const char *val)
{
  ...
  if (val[0] != '@')
    return cob_strdup (val);

  argv = (char **) calloc (3, sizeof (char *));
  orig = argv;
  argv[0] = cob_strdup (progname);
  argv[1] = cob_strdup (val);
  argv[2] = NULL;
