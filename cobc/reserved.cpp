...
static cobc_reserved * reserved_words;

static cobc_reserved default_reserved_words[] = {
	{
...
struct amendment_list {
	amendment_list *	next;	/* next pointer */
	char 		*		word;
	char 		*		alias_for;
...
amendment_list * amendmentlist = NULL;
...
static register_struct	register_list[] = {
...
static cb_intrinsic_table function_list[] = {
	{
		"ABS",				"cob_intr_abs",
...
{
	return cob_strcasecmp(((cobc_reserved *)p1)->name, ((cobc_reserved *)p2)->name);
}
...
static void
free_amendment(amendment_list * to_free)
{
							   ...
							   char ** const out_str)
{
	...
	*out_str = (char *) cobc_main_malloc(chars_to_copy + 1U);
...
{
	amendment_list * l = amendmentlist;
	size_t			length;
...
find_reserved_word(cobc_reserved to_find)
{
	return (cobc_reserved *) bsearch(&to_find, reserved_words, num_reserved_words,
...
{
	return (cobc_reserved *) bsearch(&to_find, default_reserved_words,
									 NUM_DEFAULT_RESERVED_WORDS,
									 sizeof(cobc_reserved), reserve_comp);
}

static struct cobc_reserved
get_user_specified_reserved_word(amendment_list user_reserved)
{
	struct cobc_reserved	cobc_reserved = create_dummy_reserved(NULL);
	struct cobc_reserved	to_find;
	struct cobc_reserved	* p;

	cobc_reserved.name = (char *) cobc_main_malloc(strlen(user_reserved.word) + 1);
...
static int
followed_by_addition_for_same_word(const amendment_list * addition)
{
	amendment_list * l;
...
*/
static int
try_remove_removal(amendment_list * addition)
{
	amendment_list * l = addition->next;
	amendment_list * prev = addition;
...
get_reserved_words_with_amendments(void)
{
	...
	if(cb_reserved_words == NULL) {
		...
		*/
		for(i = 0; i < NUM_DEFAULT_RESERVED_WORDS; ++i) {
			amendment = (amendment_list *)cobc_main_malloc(sizeof(amendment_list));
			amendment->word = (char *) cobc_main_malloc(strlen(default_reserved_words[i].name) + 1);
			strcpy(amendment->word, default_reserved_words[i].name);
			amendment->to_add = 1;

			/* Insert amendment at the beginning of the list. */
			amendment->next = amendmentlist;
	...
	reserved_words = (cobc_reserved *) cobc_main_malloc(num_reserved_words
					 * sizeof(cobc_reserved));

	/*
	  Populate reserved_words array with data from default_reserved_words,
	  where possible. Free each word once processed.
	*/
	for(i = 0; amendmentlist; ++i) {
		to_find = create_dummy_reserved(amendmentlist->word);
		p = find_default_reserved_word(to_find);
		if(p) {
			reserved_words[i] = *p;
			/*
			...
			*/
			...
			reserved_words[i] = get_user_specified_reserved_word(*amendmentlist);
		}

		amendment = amendmentlist->next;
		free_amendment(amendmentlist);
		amendmentlist = amendment;
	}
...
{
	system_name_struct * system_name = lookup_system_name(name, 0);
			  ...
			  const int to_add)
{
	...
	const char * equal_sign_pos = strchr(word, '=');
	int size;
	if(equal_sign_pos) {
		size = (int)(equal_sign_pos - word);
	} else {
		size = (int) strlen(word);
	}
	int context_sensitive = has_context_sensitive_indicator(word, size);
	...
	*/
	...
	amendment_list * amendment = (amendment_list *) cobc_main_malloc(sizeof(amendment_list));
	...
	amendment_list * l;
	for(l = amendmentlist; l && l->next; l = l->next);
	if(l) {
		l->next = amendment;
	} else {
		amendmentlist = amendment;
...
lookup_reserved_word(const char * name)
{
	...
	cobc_reserved * p = find_reserved_word(create_dummy_reserved(name));
	if(!p) {
		return NULL;
	}
...
add_register(const char * name_and_definition, const char * fname, const int line)
{
	...
	/* note: we don't break at space as this would kill "ADDRESS OF"
	         and "PIC 9(05) USAGE ..." */
	char * definition = (char *) strpbrk(name_and_definition, "\t:=");
	if(definition) {
		*definition++ = 0;
		/* TODO: parse definition here (and start from first non-space) */
	}

	register_struct * special_register = lookup_register(name, 1);
	if(!special_register) {
