...
cb_emit_inspect (cb_tree var, cb_tree body, const enum cb_inspect_clause clause)
{
	...
	cb_tree pst = cb_build_direct ("&inspect_st", 0);
	...
	}
	{
		...
		if (clause == CONVERTING_CLAUSE || clause == TRANSFORM_STATEMENT) {
			cb_emit (CB_BUILD_FUNCALL_2 ("cob_inspect_init_converting_mt", pst, var));
			cb_emit_list (body);
			/* no finish here */
		} else {
			cb_emit (CB_BUILD_FUNCALL_3 ("cob_inspect_init_mt", pst, var, replacing_flag));
			cb_emit_list (body);
			cb_emit (CB_BUILD_FUNCALL_1 ("cob_inspect_finish_mt", st));
		}
	}
	return;
...
cb_build_tallying_characters (cb_tree l)
{
	...
	inspect_func = NULL;
	return cb_list_add (l, CB_BUILD_FUNCALL_2 ("cob_inspect_characters_mt", st, inspect_data));
}
...
cb_build_tallying_all (void)
{
	...
	inspect_func = "cob_inspect_all_mt";
	return NULL;
}
...
cb_build_tallying_leading (void)
{
	...
	inspect_func = "cob_inspect_leading_mt";
	return NULL;
}
...
cb_build_tallying_trailing (void)
{
	...
	inspect_func = "cob_inspect_trailing_mt";
	return NULL;
}
...
cb_build_tallying_value (cb_tree x, cb_tree l)
{
	...
	return cb_list_add (l, CB_BUILD_FUNCALL_3 (inspect_func, st, inspect_data, x));
}
...
cb_build_replacing_characters (cb_tree x, cb_tree l)
{
	...
	return cb_list_add (l, CB_BUILD_FUNCALL_2 ("cob_inspect_characters_mt", st, x));
}
...
cb_build_replacing_all (cb_tree x, cb_tree y, cb_tree l)
{
	...
	return cb_list_add (l, CB_BUILD_FUNCALL_3 ("cob_inspect_all_mt", st, y, x));
}
...
cb_build_replacing_leading (cb_tree x, cb_tree y, cb_tree l)
{
	cb_tree st = cb_build_direct ("inspect_st", 0);
	(void) validate_inspect (x, y, 1);
	return cb_list_add (l, CB_BUILD_FUNCALL_3 ("cob_inspect_leading_mt", st, y, x));
}
...
cb_build_replacing_first (cb_tree x, cb_tree y, cb_tree l)
{
	cb_tree st = cb_build_direct ("inspect_st", 0);
	(void) validate_inspect (x, y, 1);
	return cb_list_add (l, CB_BUILD_FUNCALL_3 ("cob_inspect_first_mt", st, y, x));
}
...
cb_build_replacing_trailing (cb_tree x, cb_tree y, cb_tree l)
{
	cb_tree st = cb_build_direct ("inspect_st", 0);
	(void) validate_inspect (x, y, 1);
	return cb_list_add (l, CB_BUILD_FUNCALL_3 ("cob_inspect_trailing_mt", st, y, x));
}
...
cb_build_converting (cb_tree x, cb_tree y, cb_tree l)
{
	...
	if (ret) {
		...
		/* FIXME: add test case ! */
		return cb_list_add (l, CB_BUILD_FUNCALL_1 ("cob_inspect_finish_mt", st));
	}
		...
		case CB_TAG_LITERAL:
			{
				...
				return cb_list_add (l,
					CB_BUILD_FUNCALL_2 ("cob_inspect_translating_mt", st,
				...
				if ( (alph_x->alphabet_type == CB_ALPHABET_EBCDIC
				 ...
				 ||  (alph_y->alphabet_type == CB_ALPHABET_EBCDIC
				   && alph_x->alphabet_type == CB_ALPHABET_ASCII)) {
					...
					return cb_list_add (l,
						CB_BUILD_FUNCALL_2 ("cob_inspect_translating_mt", st, CB_TREE (alph_y)));
				} else {
					...
					return cb_list_add (l,
						CB_BUILD_FUNCALL_2 ("cob_inspect_translating_mt", st, CB_TREE (alph_conv)));
				}
	...
	return cb_list_add (l, CB_BUILD_FUNCALL_3 ("cob_inspect_converting_mt", st, x, y));
}
...
{
	cb_tree st = cb_build_direct ("inspect_st", 0);
	return CB_LIST_INIT (CB_BUILD_FUNCALL_1 ("cob_inspect_start_mt", st));
}
...
validate_move (cb_tree src, cb_tree dst, const unsigned int is_value, int *move_zero)
{
	...
	*move_zero = 0;
	...
	case CB_TAG_REFERENCE:
		...
		if (CB_TREE_CATEGORY (dst) == CB_CATEGORY_NATIONAL) {
			dst_size_mod /= COB_NATIONAL_SIZE;
		}
		...
		case CB_CATEGORY_FLOATING_EDITED:
			...
			case CB_CATEGORY_NATIONAL:
				...
				if (is_numeric_edited) {
					dst_size_mod = count_pic_edited (fdst);
				} else {
...
cb_emit_string (cb_tree items, cb_tree into, cb_tree pointer)
{
	...
	start = items;
	cb_emit (CB_BUILD_FUNCALL_3 ("cob_string_init_mt", pst, into, pointer));
	while (start) {
		...
		cb_emit (CB_BUILD_FUNCALL_2 ("cob_string_delimited_mt", st, dlm));
		...
		for (l = start; l != end; l = CB_CHAIN (l)) {
			...
			cb_emit (CB_BUILD_FUNCALL_2 ("cob_string_append_mt", st, cur));
		}
		if (nat > 0 && nat != nfld)
			cb_error_x (CB_TREE (current_statement),
	...
	cb_emit (CB_BUILD_FUNCALL_1 ("cob_string_finish_mt", st));
}
...
{
	cb_tree st = cb_build_direct ("unstring_st", 0);
	cb_tree pst = cb_build_direct ("&unstring_st", 0);
	...
	cb_emit (CB_BUILD_FUNCALL_4 ("cob_unstring_init_mt", pst, name, pointer,
		cb_int ((int)cb_list_length (delimited))));
	cb_emit_list (delimited);
	cb_emit_list (into);
	if (tallying) {
		cb_emit (CB_BUILD_FUNCALL_2 ("cob_unstring_tallying_mt", st, tallying));
	}
	cb_emit (CB_BUILD_FUNCALL_1 ("cob_unstring_finish_mt", st));
}
...
cb_build_unstring_delimited (cb_tree all, cb_tree value)
{
	...
	return CB_BUILD_FUNCALL_3 ("cob_unstring_delimited_mt", st, value, all);
}
...
cb_build_unstring_into (cb_tree name, cb_tree delimiter, cb_tree count)
{
	...
	return CB_BUILD_FUNCALL_4 ("cob_unstring_into_mt", st, name, delimiter, count);
}
