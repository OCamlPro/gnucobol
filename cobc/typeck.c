...
cb_validate_collating (struct cb_program *prog, cb_tree collating_sequence)
{
	...
	if (CB_ALPHABET_NAME (x)->low_val_char) {
		...
		CB_LITERAL(cb_low)->data[0] = prog->low_value = (unsigned char)CB_ALPHABET_NAME (x)->low_val_char;
		CB_LITERAL(cb_low)->all = 1;
	}

	if (CB_ALPHABET_NAME (x)->high_val_char != 255){
		cb_high = cb_build_alphanumeric_literal ("\0", (size_t)1);
		CB_LITERAL(cb_high)->data[0] = prog->high_value = (unsigned char)CB_ALPHABET_NAME (x)->high_val_char;
...
#ifdef	COB_EBCDIC_MACHINE
			load_collating_tables ();
			ap->values[n] = (int)ascii_to_ebcdic[n];
			ap->alphachr[n] = (int)ascii_to_ebcdic[n];
#else
			ap->values[n] = n;
			ap->alphachr[n] = n;
#endif
...
#else
			load_collating_tables ();
			ap->values[n] = (int)ebcdic_to_ascii[n];
			ap->alphachr[n] = (int)ebcdic_to_ascii[n];
#endif
		}
		return;
	}
