...
cb_statement	*	error_statement = NULL;
...
static expr_node *	expr_stack;			/* Expression node stack */
...
static const optim_table	bin_set_funcs[] = {
	{ NULL,				COB_OPTIM_MIN },
...
static const optim_table	bin_compare_funcs[] = {
...
static const optim_table	bin_add_funcs[] = {
...
static const optim_table	bin_sub_funcs[] = {
...
static const optim_table	align_bin_compare_funcs[] = {
	{ "cob_cmp_u8",				COB_CMP_U8 },
...
static const optim_table	align_bin_add_funcs[] = {
	{ "cob_add_u8",			COB_ADD_U8 },
...
static const optim_table	align_bin_sub_funcs[] = {
	{ "cob_sub_u8",			COB_SUB_U8 },
...
static void
cb_check_lit_subs(cb_reference * r, const int numsubs,
				  const int numindex)
{
	/* Check for DPC and non-standard separator usage */
	if(!cb_relaxed_syntax_checks || current_program->decimal_point != ',') {
		return;
	}
	if(numsubs > numindex) {
		return;
	...
	for(cb_tree l = r->subs; l; l = CB_CHAIN(l)) {
...
cb_check_field_debug(cb_tree fld)
{
	...
	if(CB_WORD_COUNT(fld) > 0) {
		...
		cb_tree z = CB_VALUE(CB_WORD_ITEMS(fld));
		if(!CB_FIELD_P(z)) {
			return;
		}
		x = cb_ref(fld);
	...
	for(cb_tree l = current_program->debug_list; l; l = CB_CHAIN(l)) {
		if(!CB_PURPOSE(l)) {
			continue;
		}
		if(x == CB_PURPOSE(l)) {
	...
	for(cb_tree l = current_statement->debug_nodups; l; l = CB_CHAIN(l)) {
		if(CB_VALUE(l) == x) {
			found = 1;
			break;
		}
	...
	for(cb_tree l = CB_REFERENCE(fld)->chain; l; l = CB_REFERENCE(l)->chain) {
		cb_tree z = cb_ref(l);
		if(z != cb_error_node) {
	...
	for(cb_tree l = CB_REFERENCE(fld)->subs; l && found < 3; l = CB_CHAIN(l), ++found) {
		switch(found) {
		case 0:
			current_statement->debug_check =
			cb_list_add(current_statement->debug_check,
...
cb_build_registers(void)
{
	...
	const char * name = cb_register_list_get_first(definition);
	while(name) {
...
{
	char * s = (char *)(CB_LITERAL(id_literal)->data);
	if(!strchr(s, ' ')) {
		return;
	}
...
cb_encode_program_id(const char * name)
{
	...
	for(const unsigned char * t = (const unsigned char *)name; *t; t++) {
	...
	*p = 0;
	...
	return(char *) cobc_check_string((char *)buff);
}
...
cb_build_section_name(cb_tree name, const int sect_or_para)
{
	...
	if(CB_WORD_COUNT(name) > 0) {
		cb_tree x = CB_VALUE(CB_WORD_ITEMS(name));
...
cb_tree
cb_build_assignment_name(cb_file * cfile, cb_tree name)
{
	if(name == cb_error_node) {
		return cb_error_node;
	}
	...
	case CB_TAG_REFERENCE: {
		...
		if(cb_assign_clause == CB_ASSIGN_MF) {
			...
			/* Check organization */
			if(strncmp(s, "S-", (size_t)2) == 0 || strncmp(s, "AS-", (size_t)3) == 0) {
				/* Skip it for now */
				s = strchr(s, '-') + 1;
			} else {
...
cb_build_index(cb_tree x, cb_tree values, const unsigned int indexed_by,
			   cb_field * qual)
{
	cb_field * f = CB_FIELD(cb_build_field(x));
...
cb_build_address(cb_tree x)
{
	...
	unsigned int refsubs = cb_list_length(r->subs);
...
cb_build_identifier(cb_tree x, const int subchk)
{
	...
	if(current_statement && !suppress_data_exceptions &&
		...
		cb_tree xr = cb_build_reference(full_name);
	...
	int numsubs = cb_list_length(r->subs);
	int refsubs = numsubs;
	...
	if(likely(!r->flag_all)) {
		...
		if(CB_EXCEPTION_ENABLE(COB_EC_BOUND_SUBSCRIPT) && f->odo_level != 0) {
			for(cb_field * p = f; p; p = p->children) {
				if(p->depending) {
					cb_tree e1 = CB_BUILD_FUNCALL_5("cob_check_odo2",
		...
		if(r->subs) {
			...
			for(cb_field * p = f; p; p = p->parent) {
				...
				if(CB_EXCEPTION_ENABLE(COB_EC_BOUND_SUBSCRIPT)) {
					if(p->depending) {
						cb_tree e1 = CB_BUILD_FUNCALL_4("cob_check_subscript2",
	...
	if(r->offset) {
		...
		if(CB_EXCEPTION_ENABLE(COB_EC_BOUND_REF_MOD)) {
			if(f->flag_any_length || !CB_LITERAL_P(r->offset) ||
				...
				cb_tree e1 = CB_BUILD_FUNCALL_4("cob_check_ref_mod",
												cb_build_cast_int(r->offset),
												r->length ? cb_build_cast_int(r->length) : cb_int1,
												f->flag_any_length ? CB_BUILD_CAST_LENGTH(v) : cb_int(pseudosize),
												CB_BUILD_STRING0(f->name));
				r->check = cb_list_add(r->check, e1);
			}
		}
...
cb_build_const_from(cb_tree x)
{
	...
	cb_define_struct * p = ppp_search_lists(CB_NAME(x));
	if(p == NULL
 ...
 */
cb_tree
cb_build_const_start(cb_field * f, cb_tree x)
{
	char			buff[32];
	...
	cb_field * target = CB_FIELD(cb_ref(x));
	if(!target) {
		return cb_error_node;
	}
	...
	for(cb_field * p = target; p; p = p->parent) {
...
cb_build_const_next(struct cb_field * f)
{
	...
	cb_field * previous = cb_get_real_field();
	...
	*/
	if(previous->level != 1) {
		int sav_min = previous->occurs_min;
		int sav_max = previous->occurs_max;
...
cb_build_length(cb_tree x)
{
	...
	cb_tree temp = cb_build_index(cb_build_filler(), NULL, 0, NULL);
...
cb_build_ppointer(cb_tree x)
{
	...
	if(CB_REFERENCE_P(x)) {
		cb_field * f = CB_FIELD_PTR(cb_ref(x));
		f->count++;
	}
...
static void
cb_validate_collating(cb_program * prog)
{
...
void
cb_validate_program_environment(cb_program * prog)
{
	/* Check ALPHABET clauses */
	/* Complicated by difference between code set and collating sequence */
	for(cb_tree l = current_program->alphabet_name_list; l; l = CB_CHAIN(l)) {
		cb_alphabet_name * ap = CB_ALPHABET_NAME(CB_VALUE(l));
		...
		for(cb_tree y = ap->custom_list; y; y = CB_CHAIN(y)) {
			pos++;
			if(count > 255) {
				unvals = pos;
				break;
			}
			cb_tree x = CB_VALUE(y);
			if(CB_PAIR_P(x)) {
				/* X THRU Y */
				...
				for(cb_tree ls = x; ls; ls = CB_CHAIN(ls)) {
					int n = get_value(CB_VALUE(ls));
					if(!CB_CHAIN(ls)) {
						lastval = n;
					}
					if(n < 0 || n > 255) {
	...
	for(cb_tree l = current_program->symbolic_char_list; l; l = CB_CHAIN(l)) {
		cb_tree y;
		if(CB_VALUE(l)) {
			y = cb_ref(CB_VALUE(l));
			if(y == cb_error_node) {
	...
	/* Check CLASS clauses */
	for(cb_tree l = current_program->class_name_list; l; l = CB_CHAIN(l)) {
		size_t dupls = 0;
		int values[256];
		memset(values, 0, sizeof(values));
		cb_class_name * cp = CB_CLASS_NAME(CB_VALUE(l));
		for(cb_tree y = cp->list; y; y = CB_CHAIN(y)) {
			cb_tree x = CB_VALUE(y);
			if(CB_PAIR_P(x)) {
				/* X THRU Y */
				int lower = get_value(CB_PAIR_X(x));
...
{
	/* Set up DEBUG-ITEM */
	cb_tree l = cb_build_reference("DEBUG-ITEM");
	cb_tree assign = cb_build_field_tree(NULL, l, NULL, CB_STORAGE_WORKING,
										 NULL, 1);
	CB_FIELD(assign)->values = CB_LIST_INIT(cb_space);
	cb_debug_item = l;

	l = cb_build_reference("DEBUG-LINE");
	cb_tree x = cb_build_field_tree(NULL, l, CB_FIELD(assign),
...
{
	cb_field * key_field = CB_FIELD_PTR(file->key);
...
void
cb_validate_program_data(cb_program * prog)
{
	current_program->report_list = cb_list_reverse(current_program->report_list);

	for(cb_tree l = current_program->report_list; l; l = CB_CHAIN(l)) {
		/* Set up LINE-COUNTER / PAGE-COUNTER */
		char buff[COB_MINI_BUFF];
		cb_report * rep = CB_REPORT(CB_VALUE(l));
		snprintf(buff, (size_t)COB_MINI_MAX, "LINE-COUNTER %s", rep->cname);
		...
		snprintf(buff, (size_t)COB_MINI_MAX, "PAGE-COUNTER %s", rep->cname);
	...
	for(cb_tree l = current_program->file_list; l; l = CB_CHAIN(l)) {
		cb_file * file = CB_FILE(CB_VALUE(l));
	...
	if(cb_assign_clause == CB_ASSIGN_MF) {
		for(cb_tree l = current_program->file_list; l; l = CB_CHAIN(l)) {
			...
			if(CB_REFERENCE_P(assign)) {
				for(cb_tree x = current_program->file_list; x; x = CB_CHAIN(x)) {
					if(!strcmp(CB_FILE(CB_VALUE(x))->name,
							   CB_NAME(assign))) {
						redefinition_error(assign);
					}
				}
				cb_field * p = check_level_78(CB_NAME(assign));
				if(p) {
					unsigned char * c = (unsigned char *)CB_LITERAL(CB_VALUE(p->values))->data;
					assign = build_literal(CB_CATEGORY_ALPHANUMERIC, c, strlen((char *)c));
			...
			if(CB_REFERENCE_P(assign) &&
				...
				CB_FIELD(x)->count++;
				cb_field * p = current_program->working_storage;
				if(p) {
					while(p->sister) {
						p = p->sister;
					}
			...
			if(CB_REFERENCE_P(assign)) {
				cb_tree x = cb_ref(assign);
				if(CB_FIELD_P(x)) {
					if(CB_FIELD(x)->level == 88) {
						cb_error_x(assign, _("ASSIGN data item '%s' is invalid"), CB_NAME(assign));
	...
	if(prog->crt_status) {
		...
		cb_tree l = cb_build_reference("COB-CRT-STATUS");
		cb_field * p = CB_FIELD(cb_build_field(l));
	...
	for(cb_tree l = cb_list_reverse(prog->reference_list); l; l = CB_CHAIN(l)) {
		cb_ref(CB_VALUE(l));
	}

	/* Check ODO items */
	for(cb_tree l = cb_depend_check; l; l = CB_CHAIN(l)) {
		cb_tree x = CB_VALUE(l);
		if(x == cb_error_node) {
			continue;
		}
		cb_field * q = CB_FIELD_PTR(x);
		cb_field * depfld;
		...
		for(cb_field * p = q; ; p = p->parent) {
			if(p->depending) {
				/* Simon: nested ODO deactivated completely, see FR #99 */
				if(odo_level) {
					cb_error_x(x,
...
void
cb_validate_program_body(cb_program * prog)
{
	/* Resolve all labels */
	cb_label * save_section = current_section;
	cb_label * save_paragraph = current_paragraph;
	for(cb_tree l = cb_list_reverse(prog->label_list); l; l = CB_CHAIN(l)) {
		cb_tree x = CB_VALUE(l);
	...
	int size = 31;
	for(cb_tree l = prog->debug_list; l; l = CB_CHAIN(l)) {
		cb_tree x = CB_VALUE(l);
		current_section = CB_REFERENCE(x)->section;
		current_paragraph = CB_REFERENCE(x)->paragraph;
		cb_tree v = cb_ref(x);
	...
	if(current_program->flag_debugging) {
		if(size != 31) {
			cb_field * f = CB_FIELD_PTR(cb_debug_contents);
			f->size = size;
			f->memory_size = size;
			size -= 31;
	...
	for(cb_tree l = prog->alter_list; l; l = CB_CHAIN(l)) {
		...
		cb_label * l1 = CB_LABEL(CB_REFERENCE(x)->value);
		cb_label * l2 = CB_LABEL(CB_REFERENCE(v)->value);
		current_section = CB_REFERENCE(x)->section;
		current_paragraph = CB_REFERENCE(x)->paragraph;
		...
		if(!aid) {
			aid = (cb_alter_id *) cobc_parse_malloc(sizeof(cb_alter_id));
...
cb_expr_init(void)
{
	if(initialized == 0) {
		...
		expr_stack = (expr_node *) cobc_main_malloc(sizeof(expr_node) * START_STACK_SIZE);
	} else {
		memset(expr_stack, 0, expr_stack_size * sizeof(expr_node));
	}
	expr_op = 0;
	expr_lh = NULL;
...
expr_chk_cond(cb_tree expr_1, cb_tree expr_2)
{
	...
	if(CB_REF_OR_FIELD_P(expr_1)) {
		cb_field * f1 = CB_FIELD_PTR(expr_1);
		if(f1->level == 88) {
			return 1;
		}
	...
	if(CB_REF_OR_FIELD_P(expr_2)) {
		cb_field * f2 = CB_FIELD_PTR(expr_2);
		if(f2->level == 88) {
			return 1;
		}
...
cb_expr_shift(int token, cb_tree value)
{
	...
	if(expr_index >= expr_stack_size) {
		...
		expr_stack = (expr_node *) cobc_main_realloc(expr_stack, sizeof(expr_node) * expr_stack_size);
	}
...
cb_build_expr(cb_tree list)
{
	...
	for(cb_tree l = list; l; l = CB_CHAIN(l)) {
		...
		default:
			...
			if(cb_warn_parentheses
				...
				} else if(op == '&' && expr_stack[expr_index - 2].token == '|') {
					cb_warning(cb_warn_parentheses,
...
decimal_compute(const int op, cb_tree x, cb_tree y)
{
	...
	default:
		...
		if(func) {
			...
			cb_error_x(current_statement, _("%s operator may be misplaced"), func);
			return;
		}
		/* LCOV_EXCL_START */
...
decimal_expand(cb_tree d, cb_tree x)
{
	...
	case CB_TAG_BINARY_OP: {
		...
		if(CB_TREE_TAG(p->y) == CB_TAG_LITERAL
			...
			cb_tree t = cb_build_decimal_literal(cb_lookup_literal(p->y, 1));
...
cb_build_mul(cb_tree v, cb_tree n, cb_tree round_opt)
{
	...
	if(CB_REF_OR_FIELD_P(v)) {
		cb_field * f = CB_FIELD_PTR(v);
		f->count++;
	}
	if(CB_REF_OR_FIELD_P(n)) {
		cb_field * f = CB_FIELD_PTR(n);
		f->count++;
	}
	cb_tree opt = build_store_option(v, round_opt);
...
build_decimal_assign(cb_tree vars, const int op, cb_tree val)
{
	if(cb_arithmetic_osvs) {
		...
		for(cb_tree l = vars; l; l = CB_CHAIN(l)) {
			if(CB_FIELD_P(cb_ref(CB_VALUE(l)))) {
				cb_field * f = CB_FIELD_PTR(CB_VALUE(l));
	...
	if(op == 0) {
		for(cb_tree l = vars; l; l = CB_CHAIN(l)) {
			...
			cb_tree s2 = cb_list_reverse(decimal_stack);
			if(!s1) {
				s1 = s2;
			} else {
...
static cb_tree
cb_build_optim_cond(cb_binary_op * p)
{
#if	0	/* RXWRXW - US */
	cb_field	* fy;
...
cb_chk_num_cond(cb_tree x, cb_tree y)
{
	...
	cb_field * fx = CB_FIELD_PTR(x);
	cb_field * fy = CB_FIELD_PTR(y);
				   ...
				   cb_tree x1, cb_tree x2, cb_tree opt)
{
	...
	for(cb_field * f1 = CB_FIELD_PTR(x1)->children; f1; f1 = f1->sister) {
		if(!f1->redefines && !f1->flag_occurs) {
			for(cb_field * f2 = CB_FIELD_PTR(x2)->children; f2; f2 = f2->sister) {
...
emit_accept_external_form(cb_tree x)
{
	...
	unsigned int found = 0;
	for(cb_field * f = CB_FIELD_PTR(x)->children; f; f = f->sister) {
		if(!f->redefines) {
			if(f->children) {
				cb_tree t = cb_build_field_reference(f, x);
				...
				if(f->flag_occurs) {
					for(int i = 1; i <= f->occurs_max; i++) {
						sprintf(buff, "%d", i);
						cb_tree n = cb_build_numeric_literal(0, buff, 0);

						cb_tree o = cb_build_field_reference(f, x);
					...
					cb_tree n = cb_build_numeric_literal(0, "1", 0);
#if 0 /* TODO: implement CGI runtime, see Patch #27 */
...
{
	cb_tree x2 = cb_check_group_name(x1);
	if(cb_validate_one(x2)) {
		return;
	}
...
static void
output_screen_from(cb_field * p, const unsigned int sisters)
{
	if(sisters && p->sister) {
...
static void
output_screen_to(cb_field * p, const unsigned int sisters)
{
	if(sisters && p->sister) {
	...
	if(type == COB_SCREEN_TYPE_FIELD && p->screen_to) {
		/* Bump reference count */
		p->count++;
		cb_emit(CB_BUILD_FUNCALL_2("cob_move", p, p->screen_to));
	}
}
...
{
	if(!pos) {
		cb_emit(CB_BUILD_FUNCALL_10("cob_field_accept2",
...
void
cb_emit_accept(cb_tree var, cb_tree pos, cb_attr_struct * attr_ptr)
{
	cb_tree fgc = NULL;
	cb_tree bgc = NULL;
	cb_tree scroll = NULL;
		...
		if((CB_REF_OR_FIELD_P(var)) &&
			...
			if(pos) {
				if(CB_LIST_P(pos)) {
					cb_tree line = CB_PAIR_X(pos);
					cb_tree column = CB_PAIR_Y(pos);
					cb_emit(CB_BUILD_FUNCALL_5("cob_screen_accept2",
...
cb_emit_accept_name(cb_tree var, cb_tree name)
{
	...
	cb_tree sys = get_system_name(CB_NAME(name));
	if(sys) {
				 ...
				 cb_tree initialize)
{
	...
	if(target1) {
		...
		cb_tree x = cb_build_numeric_literal(0, buff, 0);
			 ...
			 cb_tree convention, cb_tree newthread, cb_tree handle)
{
	...
	for(cb_tree l = par_using; l; l = CB_CHAIN(l), numargs++) {
		cb_tree x = CB_VALUE(l);
		if(x == cb_error_node) {
			error_ind = 1;
			continue;
		...
		if((CB_REFERENCE_P(x) && CB_FIELD_P(CB_REFERENCE(x)->value)) || CB_FIELD_P(x)) {
			cb_field * f = CB_FIELD_PTR(x);
			if(f->level == 88) {
	...
	if(prog_is_literal_or_prototype) {
		...
		cob_u32_t is_sys_idx = 1;
		for(const system_table * psyst = system_tab; psyst->syst_name; psyst++, is_sys_idx++) {
			if(!strcmp(entry, (const char *)psyst->syst_name)) {
				...
				if(psyst->syst_params_max < numargs) {
					cb_warning_x(COBC_WARN_FILLER, current_statement,
								 _("wrong number of CALL parameters for '%s', %d given, %d expected"),
		...
		const char * entry = CB_FIELD(CB_REFERENCE(prog)->value)->name;
...
emit_screen_display(const cb_tree x, const cb_tree pos)
{
	...
	cb_emit(CB_BUILD_FUNCALL_4("cob_screen_display2", x, line, column,
							   cb_int(line_col_zero_is_supported())));
}
				   ...
				   const cb_tree size_is, const cob_flags_t disp_attrs)
{
	...
	get_line_and_column_from_pos(pos, &line, &column);
	cb_emit(CB_BUILD_FUNCALL_8("cob_field_display2",
...
cb_emit_display(cb_tree values, cb_tree upon, cb_tree no_adv,
				...
				enum cb_display_type display_type)
{
	cb_tree		fgc;
	cb_tree		bgc;
	cb_tree		scroll;
...
cb_build_display_name(cb_tree x)
{
	...
	/* Allow direct reference to a device name (not defined as mnemonic name) */
	cb_tree sys = get_system_name(name);
	if(sys) {
		switch(CB_SYSTEM_NAME(sys)->token) {
		case CB_DEVICE_CONSOLE:
		case CB_DEVICE_SYSOUT:
...
evaluate_test(cb_tree s, cb_tree o)
{
	...
	int flag = CB_PURPOSE_INT(o);
	cb_tree x = CB_PAIR_X(CB_VALUE(o));
	cb_tree y = CB_PAIR_Y(CB_VALUE(o));
...
build_evaluate(cb_tree subject_list, cb_tree case_list, cb_tree labid)
{
	...
	cb_tree whens = CB_VALUE(case_list);
...
cb_emit_evaluate(cb_tree subject_list, cb_tree case_list)
{
	...
	cb_tree x = cb_build_direct(cobc_parse_strdup(sbuf), 0);
...
cb_emit_free(cb_tree vars)
{
	...
	int i = 1;
	for(cb_tree l = vars; l; l = CB_CHAIN(l), i++) {
		if(CB_TREE_CLASS(CB_VALUE(l)) == CB_CLASS_POINTER) {
			if(CB_CAST_P(CB_VALUE(l))) {
				cb_field * f = CB_FIELD_PTR(CB_CAST(CB_VALUE(l))->val);
...
{
	cb_tree stmt_lis = cb_check_needs_break(stmts);
				   ...
				   cb_tree replacing, cb_tree def)
{
	...
	unsigned int no_fill_init = (fillinit == NULL);
	unsigned int def_init = (def != NULL);
	for(cb_tree l = vars; l; l = CB_CHAIN(l)) {
		cb_tree x = CB_VALUE(l);
		if(!(CB_REFERENCE_P(x) && CB_FIELD_P(CB_REFERENCE(x)->value)) && !CB_FIELD_P(x)) {
			cb_error_x(current_statement, _("invalid INITIALIZE statement"));
			return;
		}
...
warning_destination(cb_tree x)
{
	...
	if(!strcmp(f->name, "RETURN-CODE") ||
		...
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE %s"),
					 f->name, f->pic->orig);
	} else if(f->usage == CB_USAGE_FLOAT) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE FLOAT"),
					 f->name);
	} else if(f->usage == CB_USAGE_DOUBLE) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE DOUBLE"),
					 f->name);
	} else if(f->usage == CB_USAGE_LONG_DOUBLE) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE FLOAT EXTENDED"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_BIN32) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE FLOAT-BINARY-7"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_BIN64) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE FLOAT-BINARY-16"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_BIN128) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE FLOAT-BINARY-34"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_DEC64) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE FLOAT-DECIMAL-16"),
					 f->name);
	} else if(f->usage == CB_USAGE_FP_DEC128) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as USAGE FLOAT-DECIMAL-34"),
					 f->name);
	} else if(f->pic) {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as PIC %s"),
					 cb_name(f), f->pic->orig);
	} else {
		cb_warning_x(COBC_WARN_FILLER, f, _("'%s' defined here as a group of length %d"),
					 cb_name(f), f->size);
	}
}
...
static int
count_pic_alphanumeric_edited(cb_field * field)
{
	/* Count number of free places in an alphanumeric edited field */
	int count = 0;
	for(cob_pic_symbol * p = field->pic->str; p->symbol != 0; ++p) {
		if(p->symbol == '9' || p->symbol == 'A' || p->symbol == 'X') {
			count += p->times_repeated;
		}
	}
	return count;
}

inline static bool posoverlapret(cb_tree src, cb_tree dst)
{
	cb_tree loc = src->source_line ? src : dst;
	if(cb_warn_pos_overlap && !suppress_warn) {
		cb_warning_x(COBC_WARN_FILLER, loc, _("overlapping MOVE may occur and produce unpredictable results"));
	}
	return true;
}

inline static bool overlapret(cb_tree src, cb_tree dst)
{
	cb_tree loc = src->source_line ? src : dst;
	if((cb_warn_overlap || cb_warn_pos_overlap) && !suppress_warn) {
		cb_warning_x(COBC_WARN_FILLER, loc, _("overlapping MOVE may produce unpredictable results"));
	}
	return true;
}
...
{
	cb_reference * sr = CB_REFERENCE_P(src) ? CB_REFERENCE(src) : NULL;
	cb_reference * dr = CB_REFERENCE_P(dst) ? CB_REFERENCE(dst) : NULL;
	...
	if(src_f == dst_f) {
		...
		if(!sr->offset || !dr->offset) {
			return overlapret(src, dst);
		}
	} else {
		/* Check basic overlapping */
		for(cb_field * f1 = src_f->children; f1; f1 = f1->sister) {
			if(f1 == dst_f) {
				return overlapret(src, dst);
			}
		}
		for(cb_field * f1 = dst_f->children; f1; f1 = f1->sister) {
			if(f1 == src_f) {
				return overlapret(src, dst);
			}
		}
...
#pragma warning(pop)
	...
	int src_off = src_f->offset;
	int dst_off = dst_f->offset;
...
int
validate_move(cb_tree src, cb_tree dst, bool is_value)
{
	int size, dst_size_mod, dst_scale_mod, src_scale_mod;
	cb_literal * l;

	cb_tree loc = src->source_line ? src : dst;
	bool is_numeric_edited = false;
	overlapping = false;
	cb_field * fdst;
	cb_field * fsrc;
	...
	case CB_TAG_LITERAL:
		...
		if(CB_TREE_CLASS(src) == CB_CLASS_NUMERIC) {
			...
			case CB_CATEGORY_ALPHABETIC:
				...
				if(warningopt) {
					cb_warning_x(COBC_WARN_FILLER, loc, _("numeric move to ALPHABETIC"));
				}
#endif
				break;
			default:
...
{
	cb_literal * l = CB_LITERAL(src);
	cb_field * f = CB_FIELD_PTR(dst);
	cb_category cat = CB_TREE_CATEGORY(dst);
	...
	if(l->all) {
		...
		unsigned char * buff = (unsigned char *) cobc_parse_malloc((size_t)f->size);
		for(i = 0; i < f->size; i++) {
	...
	if((cat == CB_CATEGORY_NUMERIC &&
		...
		if(cat == CB_CATEGORY_NUMERIC) {
			...
			/* Check all zeros */
			int n = 0;
			for(unsigned char * p = buff; p < buff + f->size; p++) {
				if(*p != '0') {
					n = 1;
					break;
				}
	...
	if((f->usage == CB_USAGE_BINARY ||
		...
		if(f->flag_binary_swap) {
			int i = (f->size - 1) + (8 * (f->pic->have_sign ? 1 : 0));
...
{
	cb_field * src_f = CB_FIELD_PTR(src);
	int src_size = cb_field_size(src);
	cb_field * dst_f = CB_FIELD_PTR(dst);
	int dst_size = cb_field_size(dst);
...
cb_build_move(cb_tree src, cb_tree dst)
{
	...
	if(validate_move(src, dst, false) < 0) {
		return cb_error_node;
	}
	...
	if(CB_REFERENCE_P(dst)) {
		/* Clone reference */
		cb_reference * x = (cb_reference *) cobc_parse_malloc(sizeof(cb_reference));
		*x = *CB_REFERENCE(dst);
		x->flag_receiving = 1;
		dst = x;
	}
...
cb_emit_move(cb_tree src, cb_tree dsts)
{
	...
	if(cb_list_length(dsts) > 1) {
		cb_reference * r = CB_REFERENCE_P(src) ? CB_REFERENCE(src) : NULL;
	...
	for(cb_tree l = dsts; l; l = CB_CHAIN(l)) {
		cb_tree x = CB_VALUE(l);
		cb_reference * r = CB_REFERENCE_P(x) ? CB_REFERENCE(x) : NULL;
...
cb_build_perform_times(cb_tree times)
{
	...
	cb_tree x = cb_build_perform(CB_PERFORM_TIMES);
...
cb_build_perform_forever(cb_tree body)
{
	...
	cb_tree x = cb_build_perform(CB_PERFORM_FOREVER);
	CB_PERFORM(x)->body = body;
	return x;
}

cb_tree
cb_build_perform_exit(cb_label * label)
{
	cb_tree x = cb_build_perform(CB_PERFORM_EXIT);
	CB_PERFORM(x)->data = label;
	return x;
}
			 ...
			 cb_tree key, cb_tree lock_opts)
{
	...
	if(current_program->flag_debugging &&
		...
		cb_tree x = cb_build_debug(cb_debug_name, f->name, NULL);
		current_statement->handler3 =
...
cb_emit_rewrite(cb_tree record, cb_tree from, cb_tree lockopt)
{
	...
	if(CB_FILE_P(cb_ref(record))) {
		...
		file = CB_FIELD(cb_ref(record))->file;
...
cb_emit_release(cb_tree record, cb_tree from)
{
	...
	cb_field * f = CB_FIELD_PTR(record);
...
static unsigned int
search_set_keys(cb_field * f, cb_tree x)
{
	if(CB_REFERENCE_P(x)) {
		x = build_cond_88(x);
		if(!x || x == cb_error_node) {
...
cb_emit_search_all(cb_tree table, cb_tree at_end, cb_tree when, cb_tree stmts)
{
	...
	cb_tree x = cb_build_search_all(table, when);
	if(!x) {
		return;
	}
...
cb_emit_set_on_off(cb_tree l, cb_tree flag)
{
	...
	for(; l; l = CB_CHAIN(l)) {
		cb_system_name * s = CB_SYSTEM_NAME(cb_ref(CB_VALUE(l)));
...
cb_emit_set_false(cb_tree l)
{
	for(; l; l = CB_CHAIN(l)) {
		...
		cb_tree ref = cb_build_field_reference(f->parent, x);
		cb_tree val = CB_VALUE(f->false_88);
		if(CB_PAIR_P(val)) {
			val = CB_PAIR_X(val);
		}
		cb_emit(cb_build_move(val, ref));
...
void
cb_emit_set_attribute(cb_tree x, const cob_flags_t val_on, const cob_flags_t val_off)
{
	if(cb_validate_one(x)) {
		return;
	}
...
cb_emit_sort_init(cb_tree name, cb_tree keys, cb_tree col)
{
	...
	for(cb_tree l = keys; l; l = CB_CHAIN(l)) {
		if(CB_VALUE(l) == NULL) {
			CB_VALUE(l) = name;
		}
		cb_ref(CB_VALUE(l));
	...
	if(CB_FILE_P(cb_ref(name))) {
		...
		cb_field * f = CB_FIELD(cb_ref(name));
...
cb_emit_sort_giving(cb_tree file, cb_tree l)
{
	...
	for(cb_tree p = l; p; p = CB_CHAIN(p)) {
		if(CB_FILE(cb_ref(CB_VALUE(p)))->organization == COB_ORG_SORT) {
			cb_error_x(current_statement,
					   _("invalid SORT GIVING parameter"));
		}
	}
	int listlen = cb_list_length(l);
	cb_tree p = CB_BUILD_FUNCALL_2("cob_file_sort_giving", cb_ref(file), l);
...
static unsigned int
check_valid_key(const cb_file * cbf, const cb_field * f)
{
	...
	cb_field * ff = cb_field_founder(f);
	cb_field * f1;
	...
	if(!f1) {
		cb_error_x(current_statement, _("invalid key item"));
		return 1;
	}

	cb_tree kfld = cb_ref(cbf->key);
	...
	for(cb_alt_key * cbak = cbf->alt_key_list; cbak; cbak = cbak->next) {
...
cb_emit_start(cb_tree file, cb_tree op, cb_tree key, cb_tree keylen)
{
	...
	cb_tree fl = cb_ref(file);
	if(fl == cb_error_node) {
		return;
	}
	cb_file * f = CB_FILE(fl);
...
cb_emit_write(cb_tree record, cb_tree from, cb_tree opt, cb_tree lockopt)
{
	...
	if(f->organization == COB_ORG_LINE_SEQUENTIAL && opt == cb_int0) {
	...
	cb_tree check_eop;
	if(current_statement->handler_type == EOP_HANDLER && current_statement->ex_handler) {
		check_eop = cb_int1;
	} else {
		check_eop = cb_int0;
	}
...
cb_build_write_advancing_lines(cb_tree pos, cb_tree lines)
{
	...
	cb_tree e = cb_build_binary_op(cb_int(opt), '+', lines);
...
cb_build_write_advancing_mnemonic(cb_tree pos, cb_tree mnemonic)
{
	...
	int token = CB_SYSTEM_NAME(cb_ref(mnemonic))->token;
	int opt;
	switch(token) {
...
cobc_init_typeck(void)
{
	...
	for(const unsigned char * p = pvalid_char; *p; ++p) {
		valid_char[*p] = 1;
	}
