...
cb_text_list *	cb_include_list = NULL;
cb_text_list *	cb_intrinsic_list = NULL;
cb_text_list *	cb_extension_list = NULL;
cb_text_list *	cb_static_call_list = NULL;
cb_text_list *	cb_early_exit_list = NULL;
...
list_files	* cb_current_file = NULL;
#define			LCL_NAME_LEN 80
#define			LCL_NAME_MAX (LCL_NAME_LEN - 1)

/* compilation date/time of current source file */
cob_time	current_compile_time = { 0 };
tm			current_compile_tm = { 0 };

#if	0	/* RXWRXW - source format */
	char 	*		source_name = NULL;
#endif

enum cb_format	cb_source_format = CB_FORMAT_FIXED;	/* Default format */
int			cb_text_column;
int			cb_id = 0;
int			cb_pic_id = 0;
int			cb_attr_id = 0;
...
cb_exception cb_exception_table[] = {
	{NULL, 0, 0},		/* CB_EC_ZERO */
...
static cb_define_struct *	cb_define_list = NULL;

static cobc_mem_struct *	cobc_mainmem_base = NULL;
static cobc_mem_struct *	cobc_parsemem_base = NULL;
static cobc_mem_struct *	cobc_plexmem_base = NULL;
...
static const option long_options[] = {
	...
	{"free",			CB_NO_ARG, (int *) & cb_source_format, CB_FORMAT_FREE},
	{"fixed",			CB_NO_ARG, (int *) & cb_source_format, CB_FORMAT_FIXED},
...
static void	print_program_code(list_files *, int);
...
{
	if(save_temps_dir) {
		delete [] save_temps_dir;
		save_temps_dir = NULL;
	}
	if(cobc_list_dir) {
		delete [] cobc_list_dir;
		cobc_list_dir = NULL;
	}
	if(cobc_list_file) {
		delete [] cobc_list_file;
		cobc_list_file = NULL;
	}
	if(cb_listing_file_struct) {
		delete cb_listing_file_struct;
		cb_listing_file_struct = NULL;
	}
	if(cobc_run_args) {
		delete [] cobc_run_args;
		cobc_run_args = NULL;
	}
	for(cobc_mem_struct * reps = cobc_plexmem_base; reps;) {
		cobc_mem_struct * repsl = reps;
		reps = reps->next;
		delete [](char *) repsl;
	}
	cobc_plexmem_base = NULL;
	for(cobc_mem_struct * reps = cobc_parsemem_base; reps;) {
		cobc_mem_struct * repsl = reps;
		reps = reps->next;
		delete [](char *) repsl;
	}
	cobc_parsemem_base = NULL;
	for(cobc_mem_struct * reps = cobc_mainmem_base; reps;) {
		cobc_mem_struct * repsl = reps;
		reps = reps->next;
		delete [](char *) repsl;
	}
...
{
	cobc_mem_struct * m = (cobc_mem_struct *) new char[sizeof(cobc_mem_struct) + size];
	memset(m, 0, sizeof(cobc_mem_struct) + size);
	m->next = cobc_mainmem_base;
	m->memptr = (char *)m + sizeof(cobc_mem_struct);
	m->memlen = size;
	cobc_mainmem_base = m;
	return m->memptr;
}
...
cobc_main_strdup(const char * dupstr)
{
	...
	size_t n = strlen(dupstr);
	char * p = (char *) cobc_main_malloc(n + 1);
	memcpy(p, dupstr, n + 1);
	return p;
}
...
cobc_main_realloc(void * prevptr, const size_t size)
{
	...
	cobc_mem_struct * prev = NULL;
	cobc_mem_struct * curr;
...
cobc_parse_strdup(const char * dupstr)
{
	...
	size_t n = strlen(dupstr);
	char * p = (char *) cobc_parse_malloc(n + 1);
	memcpy(p, dupstr, n + 1);
	return p;
}
...
cobc_plex_strdup(const char * dupstr)
{
	...
	size_t n = strlen(dupstr);
	char * p = (char *) cobc_plex_malloc(n + 1);
	memcpy(p, dupstr, n + 1);
	return p;
}
...
cobc_check_string(const char * dupstr)
{
	...
	for(strcache * s = base_string; s; s = s->next) {
		if(!strcmp(dupstr, (const char *)s->val)) {
			return s->val;
		}
	}
	strcache * s = (strcache *) cobc_main_malloc(sizeof(strcache));
	s->next = base_string;
...
static cb_text_list *
cb_text_list_add(cb_text_list * list, const char * text)
{
	cb_text_list * p = (cb_text_list *) cobc_main_malloc(sizeof(cb_text_list));
...
static cb_text_list *
cb_text_list_chk(cb_text_list * list, const char * text)
{
	for(cb_text_list * p = list; p; p = p->next) {
...
static unsigned int
cobc_set_value(cb_define_struct * p, const char * value)
{
	if(!value) {
...
#endif
		size = strlen(value);
		p->value = (char *) cobc_main_malloc(size + 4U);
...
{
	const void ** tptr = (const void **)p2;
	return strcmp((const char *) p1, (const char *) *tptr);
}
...
size_t
cobc_check_valid_name(const char * name, const cobc_name_type prechk)
{
	size_t len = 0;
...
cobc_chk_buff_size(const size_t bufflen)
{
	if(bufflen >= cobc_buffer_size) {
		...
		cobc_buffer = (char *) cobc_main_realloc(cobc_buffer, cobc_buffer_size);
	}
}
...
cb_define_list_add(cb_define_struct * list, const char * text)
{
	char * x = cobc_strdup(text);
	char * s = strtok(x, "=");
	...
	cb_define_struct * p = (cb_define_struct *) cobc_main_malloc(sizeof(cb_define_struct));
	p->next = NULL;
	p->name = (char *) cobc_check_string(s);
	p->deftype = PLEX_DEF_NONE;
	s = strtok(NULL, "");
...
cobc_stradd_dup(const char * str1, const char * str2)
{
	...
	char * p = (char *) cobc_main_malloc(m + n + 1);
	memcpy(p, str1, m);
	memcpy(p + m, str2, n);
	return p;
}
			 ...
			 const char * s3)
{
	...
	if(calcsize >= *cursize) {
		...
		*var = (char *) cobc_main_realloc(*var, *cursize);
	}
	strcat(*var, s1);
	if(s2) {
		strcat(*var, s2);
...
cobc_clean_up(const int status)
{
	...
	for(filename * fn = file_list; fn; fn = fn->next) {
		for(local_filename * lf = fn->localfile; lf; lf = lf->next) {
...
set_listing_date(void)
{
	...
	char * time_buff = asctime(&current_compile_tm);
	if(!time_buff) {
		/* LCOV_EXCL_START */
...
cobc_mem_handler()
{
	cobc_err_msg(_("cannot allocate memory"));
	cobc_abort_terminate();
}
...
cobc_print_version(void)
{
	...
	printf(_("Written by %s\n"), "Keisuke Nishida, Roger While, Ron Norman, Simon Sobisch, Edward Hart, Sergey Kashyrin");
...
cobc_var_print(const char * msg, const char * val, const unsigned int env)
{
	if(!env) {
		...
		printf("  %s: ", _("env"));
		int lablen = CB_IMSG_SIZE - 2 - (int)strlen(_("env")) - 2;
...
#if defined(COB_64_BIT_POINTER) || defined(_WIN64)
...
process_command_line(const int argc, char ** argv)
{
	...
	while((c = cob_getopt_long_long(argc, argv, short_options,
		...
		case 'o':
			...
			output_name_buff = (char *) cobc_main_malloc(osize + 32U);
			break;
		...
		{
			char * conf_label = (char *) cobc_main_malloc(COB_MINI_BUFF);
			char * conf_entry = new char[COB_MINI_BUFF - 2];
		...
		case 'D':
			...
			{
				cb_define_struct * p = cb_define_list_add(cb_define_list, cob_optarg);
				if(!p) {
					cobc_err_exit(COBC_INV_PAR, "-D");
				}
				cb_define_list = p;
...
#undef	CB_NOWARNDEF
	...
	if(cobc_wants_debug) {
		for(int i = 1; i < COB_EC_MAX; ++i) {
			CB_EXCEPTION_ENABLE((cob_exception_id)i) = 1;
		}
		if(verbose_output > 1) {
...
static cb_program *
program_list_reverse(cb_program * p)
{
	cb_program * next;
	cb_program * last = NULL;
	for(; p; p = next) {
		next = p->next_program;
		p->next_program = last;
		last = p;
...
process_env_copy_path(const char * p)
{
	...
	char * value = cobc_strdup(p);

	/* Tokenize for path sep. */
	char * token = strtok(value, PATHSEP_STR);
	while(token) {
		struct stat	st;
...
static filename *
process_filename(const char * fname)
{
	int file_is_stdin;
	if(strcmp(fname, COB_DASH) == 0) {
	...
	if(strchr(fname, '.') != NULL) {
		...
		if(!file_is_stdin && access(fname, R_OK) != 0) {
	...
	char * fbasename = file_basename(fname);
	const char * extension = file_extension(fname);
	...
	filename * fn = (filename *) cobc_main_malloc(sizeof(filename));
	...
	/* Set source filename */
	fn->source = cobc_main_strdup(fname);
	...
	if(!fn->need_preprocess) {
		...
		fn->preprocess = (char *) cobc_main_malloc(COB_FILE_MAX);
	...
	if(!fn->need_translate) {
		...
		fn->translate = cobc_main_strdup(output_name);
			  ...
			  cb_compile_level == CB_LEVEL_TRANSLATE) {
		fn->translate = cobc_stradd_dup(fbasename, ".cpp");
	} else {
		fn->translate = (char *) cobc_main_malloc(COB_FILE_MAX);
		cob_temp_name((char *)fn->translate, ".cpp");
	}
	...
	if(!fn->need_assemble) {
		...
		fn->object = (char *) cobc_main_malloc(COB_FILE_MAX);
	...
	if(cobc_gen_listing == 1) {
		if(cobc_list_file) {
			...
			char * listptr = (char *) cobc_main_malloc(fsize);
...
line_contains(char * line_start, char * line_end, char * search_patterns)
{
	...
	int pattern_start = 0;
	int full_length = (int)strlen(search_patterns) - 1;
	for(int pattern_end = 0; pattern_end < (int) strlen(search_patterns); pattern_end++) {
		if(search_patterns[pattern_end] == PATTERN_DELIM) {
			...
			for(char * line_pos = line_start; line_pos + pattern_length <= line_end; line_pos++) {
...
process(char * cmd)
{
	...
	char * buffptr = new char[COB_LARGE_BUFF];
	if(name != NULL) {
...
static int
process_filtered(const char * cmd, filename * fn)
{
	...
	int i;
	for(i = (int) fn->translate_len; i > 0; i--) {
		if(fn->translate[i - 1] == '\\' || fn->translate[i - 1] == '/') {
			break;
		}
	}

	/* TODO: change to std::string */
	char * search_pattern = new char[fn->translate_len - i + 3];
	sprintf(search_pattern, "%s\n%c", fn->translate + i, PATTERN_DELIM);
	char * search_pattern2 = NULL;
	if(cb_compile_level > CB_LEVEL_ASSEMBLE) {
		search_pattern2 = new char[2 * (strlen(output_name_temp) + 5) + 1];
	...
	for(char * line_start = fgets(read_buffer, COB_FILE_BUFF - 1, pipe); line_start != NULL; line_start = fgets(read_buffer, COB_FILE_BUFF - 1, pipe)) {
		/* read one line from buffer, returning line end position */
		char * line_end = line_start + strlen(line_start);
	...
	if(search_pattern2 != NULL) {
		delete [] search_pattern2;
	}

	if(!output_name) {
		delete [] output_name_temp;
	}
...
process(const char * cmd)
{
	...
	if(likely(strchr(cmd, '$') == NULL)) {
		...
		size_t clen = strlen(cmd) + 64U + 6U;
		buffptr = new char[clen];
		char * p = buffptr;
		/* Quote '$' */
		for(; *cmd; ++cmd) {
			if(*cmd == '$') {
...
preprocess(filename * fn)
{
	...
	cb_format save_source_format = cb_source_format;
	int save_fold_copy = cb_fold_copy;
	int save_fold_call = cb_fold_call;
...
set_picture(struct cb_field * field, char * picture, int picture_len)
{
	...
	usage_len = (int) strlen(picture_usage);
...
{
	int i;
	for(i = (int) strlen(str) - 1; i && isspace(str[i] & 0x0FF); i--) {
		str[i] = '\0';
	}
	return i;
}
...
static void
print_88_values(cb_field * field)
{
...
static void
print_fields(cb_field * top, int * found)
{
	int	first = 1;
	int	get_cat;
	int	got_picture;
	...
	for(; top; top = top->sister) {
		...
		if(get_cat) {
			set_category(top->category, top->usage, type);
...
cobc_xref_call(const char * name, const int line, const int is_ident, const int is_sys)
{
	...
	elem = (cb_call_elem *) cobc_parse_malloc(sizeof(struct cb_call_elem));
...
{
	for(cb_tree l = file_list_p; l; l = CB_CHAIN(l)) {
		pd_off = sprintf(print_data, "%-30.30s %-6u ",
						 CB_FILE(CB_VALUE(l))->name,
...
print_program_trailer(void)
{
	...
	case 1:
		...
		break;
	default:
		snprintf(print_data, CB_PRINT_LEN, _("%d warnings in compilation group"), warningcount);
		print_program_data(print_data);
		break;
	}
	switch(errorcount) {
	...
	case 1:
		...
		break;
	default:
		snprintf(print_data, CB_PRINT_LEN, _("%d errors in compilation group"), errorcount);
		print_program_data(print_data);
		break;
	}
...
get_next_token(char * bp, char * token, char * term)
{
	...
	do {
		...
		while(*bp) {
			...
			if(*bp == '.' && isdigit(*(bp + 1) & 0xFF)) {
				;
			} else if(isspace(*bp & 0xFF) || *bp == ',' || *bp == '.' || *bp == ';') {
				term[0] = *bp++;
				break;
			}
			*token++ = *bp++;
...
get_next_listing_line(FILE * fd, char ** pline, int fixed)
{
	...
	if(*pline == NULL) {
		*pline = new char[CB_LINE_LENGTH + 2];
	}
	out_line = *pline;
...
line_has_page_eject(char * line, const enum cb_format source_format)
{
	if(source_format != CB_FORMAT_FREE && line[CB_INDICATOR] == '/') {
		...
		char * directive_start = get_directive_start(line, source_format);
...
line_has_listing_statement(char * line, const enum cb_format source_format)
{
	...
	char * curr_pos = get_first_nonspace(line, source_format);
	...
	for(err = first_error; err; err = err->next) {
		if(err->line == line_num) {
			...
			if(pd_off >= max_chars_on_line) {
				...
				print_data[pd_off] = '\0';
				print_program_data(print_data);
				msg_off = (int) strlen(err->prefix);
				pd_off = (int) strlen(print_data) - msg_off;
				if(msg_off < 2) {
					msg_off = 2;
				}
...
static void
print_line(list_files * cfile, char * line, int line_num, int in_copy)
{
				...
				int first_idx, int last_idx, int first_col, int fixed)
{
	...
	for(line_idx = first_idx; line_idx < last_idx; line_idx++) {
		if(!fixed) {
			last_col = (int) strlen(pline[line_idx]) - 1;
		}
...
*/
static void
adjust_line_numbers(list_files * cfile, int line_num, int adjust)
{
	list_files * cur;
	list_replace	* rep;
	list_error	* err;
						   ...
						   int * const pline_cnt, int line_num)
{
	...
	if(pline[*pline_cnt + 1] == NULL) {
		pline[*pline_cnt + 1] = new char[CB_LINE_LENGTH + 2];
	}
								  ...
								  char * newline, int first_col, const int last)
{
	...
	char * new_line_ptr = get_next_token(newline, new_token, token_terminator);
	...
	*/
	...
	/* For each line,  */
	for(int out_line = 0; out_line < *pline_cnt; out_line++) {
		int force_next_line = 0;
		int out_col = first_col;

		/* Add as many token as possible to the current line. */
		while(new_line_ptr && !force_next_line) {
			int new_token_len = (int) strlen(new_token);
...
static int
print_replace_text(list_files * cfile, FILE * fd,
				   list_replace * rep, char * pline[CB_READ_AHEAD],
...
next_rec:
		...
		from_token_len = (int) strlen(ftoken);
...
static void
remove_replace_entries_before_line(list_files * cfile, const int line_num)
{
	while(cfile->replace_head && cfile->replace_head->firstline < line_num) {
		list_replace * rep = cfile->replace_head;
...
static void
deep_copy_list_replace(list_replace * src, list_files * dst_file)
{
	list_replace * copy = new list_replace;
	memcpy(copy, src, sizeof(list_replace));
	if(src->to) {
...
*/
static int
print_replace_main(list_files * cfile, FILE * fd,
...
*/
static void
print_program_code(list_files * cfile, int in_copy)
{
	FILE		*	fd = NULL;
	list_replace	* rep;
	list_files	* cur;
	...
	if(cfile->err_head) {
		cfile->err_head = list_reverse(cfile->err_head);
	}
	cfile->listing_on = 1;
#ifdef DEBUG_REPLACE
	struct list_skip * skip;
...
print_program_listing(void)
{
	...
	delete [] cb_listing_file_struct->name;
...
process_translate(filename * fn)
{
	...
	cb_program * p = current_program;
	for(cb_program * q = p; q; q = q->next_program) {
		q->global_file_list = cb_list_reverse(q->global_file_list);
		if(q->nested_level) {
			for(cb_program * r = q->next_program; r; r = r->next_program) {
				if(r->nested_level >= q->nested_level) {
					continue;
				}
				for(int i = COB_OPEN_INPUT; i <= COB_OPEN_EXTEND; ++i) {
					handler_struct * hstr1 = &q->global_handler[i];
					handler_struct * hstr2 = &r->global_handler[i];
...
/////////////////////////// SKA //////////////////////
	for(cb_program * q = p; q; q = q->next_program) {
		...
		externalize_tree(q, &q->working_storage, true);
...
/////////////////////////// SKA //////////////////////
	...
	ret = 1;
	for(cb_program * q = p; q; q = q->next_program, ret++) {
		local_filename * lf = (local_filename *) cobc_main_malloc(sizeof(local_filename));
		lf->local_name = (char *) cobc_main_malloc(fn->translate_len + 12U);
	...
	for(cb_program * q = p; q; q = q->next_program) {
		int i = q->nested_level;
		for(nested_list	* nlp = q->common_prog_list; nlp; nlp = nlp->next) {
			for(cb_program * r = q->next_program; r; r = r->next_program) {
				if(r->nested_level <= i) {
					break;
				}
	...
	for(cb_program * q = p; q; q = q->next_program) {
		...
		if(unlikely(fclose(q->local_include->local_fp) != 0)) {
			cobc_terminate(q->local_include->local_name);
		}
...
process_compile(filename * fn)
{
	...
	size_t bufflen = cobc_cc_len + cobc_cflags_len
...
static int
process_assemble(filename * fn)
{
	size_t bufflen = cobc_cc_len + cobc_cflags_len + fn->object_len
	...
	if(name[0] != '/') {
		char * p = cobc_main_malloc(COB_LARGE_BUFF);
...
static int
process_module_direct(filename * fn)
{
	char * name;
...
#ifdef	_MSC_VER
	char * exename = cobc_stradd_dup(name, ".dll");
#endif
	...
	int ret = (verbose_output > 1) ? process(cobc_buffer) : process_filtered(cobc_buffer, fn);
	/* Embedding manifest */
	if(ret == 0) {
		sprintf(cobc_buffer,
	...
	if(strstr(fn->source, cobc_buffer) == NULL)	{
		cobc_check_action(cobc_buffer);
	}
...
process_library(filename * l)
{
	...
	size_t size = strlen(name);
	size_t bufflen = cobc_cc_len + cobc_shared_opt_len
					 + cobc_pic_flags_len + cobc_export_dyn_len
	...
	sprintf(cobc_buffer, gflag_set ?
			...
			manilink, cobc_ldflags, cobc_lib_paths, cobc_libs);
	int ret = (verbose_output > 1) ? process(cobc_buffer) : process_filtered(cobc_buffer, l);
	/* Embedding manifest */
	if(ret == 0) {
		sprintf(cobc_buffer,
				"%s /manifest \"%s.manifest\" /outputresource:\"%s\";#2",
...
#ifdef	_MSC_VER
	char * exename = cobc_stradd_dup(name, ".exe");
#endif

	size_t size = strlen(name);
	size_t bufflen = cobc_cc_len + cobc_export_dyn_len
...
main(int argc, char ** argv)
{
	...
	std::set_new_handler(cobc_mem_handler);

	cb_saveargc = argc;
	cb_saveargv = argv;

	/* General buffers */
	cobc_buffer = (char *) cobc_main_malloc((size_t)COB_LARGE_BUFF);
	cobc_buffer_size = COB_LARGE_MAX;
	basename_buffer = (char *) cobc_main_malloc((size_t)COB_MINI_BUFF);
	basename_len = COB_MINI_MAX - 16;

	cobc_libs = (char *) cobc_main_malloc((size_t)COB_SMALL_BUFF);
	cobc_lib_paths = (char *) cobc_main_malloc((size_t)COB_SMALL_BUFF);
	cobc_cflags = (char *) cobc_main_malloc((size_t)COB_MINI_BUFF);
	cobc_ldflags = (char *) cobc_main_malloc((size_t)COB_MINI_BUFF);
	cobc_include = (char *) cobc_main_malloc((size_t)COB_MINI_BUFF);
	...
	cb_flag_computed_goto = 0;
#elif	defined(__xlC__)
	cb_flag_computed_goto = 0;
	COBC_ADD_STR(cobc_cflags, " -qlanglvl=extended", NULL, NULL);
	...
	if(cb_listing_outputfile) {
		...
		cb_listing_file_struct = new list_files;
	}
	...
	unsigned int local_level = 0;

	/* Set up file parameters, if any are missing: abort */
	while(iargs < argc) {
		filename * fn = process_filename(argv[iargs++]);
		if(!fn) {
			cobc_clean_up(1);
			return 1;
		}
	...
	for(filename * fn = file_list; fn; fn = fn->next) {
		...
		if(fn->need_translate) {
			...
			/* Free parse memory */
			for(cobc_mem_struct * mptr = cobc_parsemem_base; mptr;) {
				cobc_mem_struct * mptrt = mptr;
				mptr = mptr->next;
				delete [](char *) mptrt;
			}
			cobc_parsemem_base = NULL;
	...
	cobc_objects_buffer = (char *) cobc_main_malloc(cobc_objects_len);
